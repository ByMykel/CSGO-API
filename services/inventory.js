...
const formatInventoryData = ({ skins, crates, collectibles, stickers, graffiti, musicKits }) => {
    const items = {}
    ...
    collectibles.forEach((collectible) => {
        ...
        if (!items['collectibles'][collectible.id.replace('collectible-', '')]) items['collectibles'][collectible.id.replace('collectible-', '')] = {};
    ...
    stickers.forEach((sticker) => {
        ...
        if (!items['stickers'][sticker.id.replace('sticker-', '')]) items['stickers'][sticker.id.replace('sticker-', '')] = {};
    ...
    graffiti.forEach((graffiti) => {
        ...
        if (!items['graffiti'][graffiti.id.replace('graffiti-', '')]) items['graffiti'][graffiti.id.replace('graffiti-', '')] = {};
    ...
    musicKits.forEach((musicKit) => {
        if (!musicKit.id.includes('_st')) {
            ...
            if (!items['music_kits'][musicKit.id.replace('music_kit-', '')]) items['music_kits'][musicKit.id.replace('music_kit-', '')] = {};
...
export const getInventory = async () => {
    ...
    try {
        ...
        const inventory = formatInventoryData({ skins, crates, collectibles, stickers, graffiti, musicKits });
